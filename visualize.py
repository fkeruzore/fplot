...
def load_fpl_data(csv_path):
    ...
    sorted_indices = sorted(range(len(data['gw'])), key=lambda i: data['gw'][i])
    for key in data:
...
def plot_rank_evolution(csv_path):
    ...
    for i, (gw, tc) in enumerate(zip(data['gw'], data['tc'])):
        if tc > 0:
            ax.axvspan(gw - 0.25, gw + 0.25, color='C2', alpha=0.3,
                      label='Hits' if i == 0 or data['tc'][i-1] == 0 else '',
                      lw=0, ec=None, zorder=0)

    # 2. Plot Overall Rank line
    ax.plot(data['gw'], data['or_vals'], color='C0', linewidth=2,
            marker='o', markersize=8, label='OR', mec="w", mew=1, zorder=4)

    # 3. Plot final/current OR horizontal line
    ax.axhline(y=final_or, color='C0', linestyle='--', linewidth=1.5,
               label=or_label, zorder=3)

    # 4. Plot Gameweek Ranks (only where data exists)
    gw_with_gwr = [gw for gw, gwr in zip(data['gw'], data['gwr']) if gwr is not None]
    gwr_valid = [gwr for gwr in data['gwr'] if gwr is not None]

    if gw_with_gwr:
        ax.plot(gw_with_gwr, gwr_valid, color='C1', marker='D',
                markersize=8, linestyle='none', label='GWR', mec="w", mew=1, zorder=2)

    # 5. Plot mean GWR horizontal line
    if mean_gwr is not None:
        ax.axhline(y=mean_gwr, color='C1', linestyle='--', linewidth=1.5,
                   label='(Mean)', zorder=1)
